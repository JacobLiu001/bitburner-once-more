  ...
  while (true) {
    try {
      ...
      for (const [header, show, formattedValue, toolTip] of hudData) {
        ...
        const paddedValue =
          formattedValue == null ? null : " " + formattedValue?.trim();
...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
    ...
    if (9 in dictSourceFiles || 9 == bitNode) {
      ...
      if (hashes[1] > 0) {
        val1.push(
          ...
          `Current Hashes ${hashes[0].toLocaleString(
            "en",
          )} / Current Hash Capacity ${hashes[1].toLocaleString("en")}`,
        );
        ...
        const liquidatingHashes = await getNsDataThroughFile(
          ns,
          `ns.ps('home').filter(p => p.filename == ns.args[0] && (p.args.includes('--liquidate') || p.args.includes('-l')))`,
          "/tmp/insight/hash-liquidation-scripts.txt",
          [spendHashesScript],
        );
        if (liquidatingHashes.length > 0)
          val2.push(
            true,
            "Liquidating",
            `You have a script running that is selling hashes as quickly as possible ` +
              `(PID ${
                liquidatingHashes[0].pid
              }: ${spendHashesScript} ${liquidatingHashes[0].args.join(" ")})`,
          );
      }
    }
  ...
  const totalScriptInc = await getNsDataThroughFile(
    ns,
    "ns.getTotalScriptIncome()",
  );
  const totalScriptExp = await getNsDataThroughFile(
    ns,
  ...
  hudData.push([
    "Scr Inc",
    true,
    formatMoney(totalScriptInc[0], 3, 2) + "/sec",
    "Total 'instantaneous' income per second being earned across all scripts running on all servers.",
  ]);
  hudData.push([
    "Scr Exp",
    true,
    formatNumberShort(totalScriptExp, 3, 2) + "/sec",
    "Total 'instantaneous' hack experience per second being earned across all scripts running on all servers.",
  ]);
    ...
    if ((2 in dictSourceFiles || 2 == bitNode) && gangInfo) {
      ...
      val1.push(
        ...
        `Gang (${gangInfo.faction}) income per second while doing tasks.` +
          `\nIncome: ${formatMoney(
            gangInfo.moneyGainRate * 5,
          )}/sec (${formatMoney(gangInfo.moneyGainRate)}/tick)` +
          `  Respect: ${formatNumberShort(
            gangInfo.respect,
          )} (${formatNumberShort(gangInfo.respectGainRate)}/tick)` +
      ...
      val2.push(
        ...
        `How your gang is currently doing in territory warfare. Starts at 14.29%\n` +
          `Gang: ${gangInfo.faction} ${
            gangInfo.isHacking ? "(Hacking)" : "(Combat)"
          }  ` +
  ...
  // Show Karma if we're not in a gang yet
  {
    ...
    if (
      ...
      if (2 in dictSourceFiles && 2 != bitNode && !gangInfo)
        karmaShown += "/54k"; // Display karma needed to unlock gangs ouside of BN2
      val.push(
        true,
        karmaShown,
  ...
  // Show Bladeburner Rank and Skill Points
  {
    ...
    if (
      ...
      const bbSP = await getNsDataThroughFile(
        ns,
        "ns.bladeburner.getSkillPoints()",
      );
      val1.push(
        true,
        formatSixSigFigs(bbRank),
        "Your current bladeburner rank",
      );
      val2.push(
        true,
        formatSixSigFigs(bbSP),
        "Your current unspent bladeburner skill points",
      );
    } else {
      val1.push(false);
      val2.push(false);
  ...
  // Show various server / RAM utilization stats
  {
    ...
    if (!options["hide-RAM-utilization"]) {
      ...
      const hnServers = servers.filter(
        (s) =>
          s.hostname.startsWith("hacknet-server-") ||
          s.hostname.startsWith("hacknet-node-"),
      );
      const nRooted = servers.filter((s) => s.hasAdminRights).length;
      const nPurchased = servers.filter(
        (s) => s.hostname != "home" && s.purchasedByPlayer,
      ).length; // "home" counts as purchased by the game
      // Add Server count.
      val1.push(
        true,
        `${servers.length}/${nRooted}/${nPurchased}`,
        `The number of servers on the network (${servers.length}) / ` +
          `number rooted (${nRooted}) / number purchased ` +
          (hnServers.length > 0
            ? `(${nPurchased - hnServers.length} servers + ${
                hnServers.length
              } hacknet servers)`
            : `(${nPurchased})`),
      );
      ...
      val2.push(
        true,
        `${formatRam(home.maxRam)} ${(
          (100 * home.ramUsed) /
          home.maxRam
        ).toFixed(1)}%`,
        `Shows total home RAM (and current utilization %)\nDetails: ${home.cpuCores} cores and using ` +
          `${formatRam(home.ramUsed, true)} of ${formatRam(
            home.maxRam,
            true,
          )} (${formatRam(home.maxRam - home.ramUsed, true)} free)`,
      );
      ...
      const fileredServers = servers.filter(
        (s) => s.hasAdminRights && !hnServers.includes(s),
      );
      const [sMax, sUsed] = fileredServers.reduce(
        ([tMax, tUsed], s) => [tMax + s.maxRam, tUsed + s.ramUsed],
        [0, 0],
      );
      const [hMax, hUsed] = hnServers.reduce(
        ([tMax, tUsed], s) => [tMax + s.maxRam, tUsed + s.ramUsed],
        [0, 0],
      );
      ...
      let toolTip =
        ...
        (9 in dictSourceFiles || 9 == bitNode
          ...
          : "") +
        `\nUsing ${formatRam(tUsed, true)} of ${formatRam(
          tMax,
          true,
        )} (${formatRam(tMax - tUsed, true)} free) across all servers`;
      if (hMax > 0)
        toolTip +=
          `\nUsing ${formatRam(sUsed, true)} of ${formatRam(
            sMax,
            true,
          )} (${formatRam(sMax - sUsed, true)} free) excluding  hacknet` +
          `\nUsing ${formatRam(hUsed, true)} of ${formatRam(
            hMax,
            true,
          )} (${formatRam(hMax - hUsed, true)} free) of hacknet servers`;
...
function addCSS(doc) {
  ...
  const hudParent =
    doc.getElementsByClassName(`MuiCollapse-root`)[0].parentElement;
  if (hudParent) hudParent.style.zIndex = 1e4; // Tail windows start around 1500, this should keep the HUD above them
  doc.head.insertAdjacentHTML(
    "beforeend",
    css(hudParent ? eval("window").getComputedStyle(hudParent) : null),
  );
}
