...
export function main(ns) {
  ...
  if (!terminalInput)
    throw new Error("This script must be run while the terminal is visible.");
  ...
  function createServerEntry(serverName) {
    ...
    return (
      `<span id="${serverName}">` +
      `<a class="server${
        factionServers.includes(serverName) ? " faction" : ""
      }` +
      ...
      (server.purchasedByPlayer
        ...
        : ` <span class="hack ${
            canHack ? "green" : "red"
          }">(${requiredHackLevel})</span>`) +
      `${
        shouldBackdoor
          ? ` <span class="${
              factionServers.includes(serverName) ? "faction " : ""
            }backdoor">[<a>backdoor</a>]</span>`
          : ""
      }` +
      ` ${contracts
        .map((c) => `<span class="cct" title="${c}">@</span>`)
        .join("")}` +
      (showStats
        ? ` <span class="serverStats">... ` +
          ...
          (
            ((server.moneyMax ?? 0 > 0)
              ? `${formatMoney(server.moneyAvailable ?? 0, 4, 1).padStart(
                  7,
                )} / `
              : "") + `${formatMoney(server.moneyMax ?? 0, 4, 1).padStart(7)} `
          ).padEnd(18) +
          `Sec: ${formatNumber(server.hackDifficulty ?? 0, 0, 0).padStart(
            3,
          )}/${formatNumber(server.minDifficulty ?? 0, 0, 0)} `.padEnd(13) +
          `RAM: ${formatRam(server.maxRam ?? 0)
            .replace(" ", "")
            .padStart(6)}` +
          ((server.maxRam ?? 0 > 0)
            ? ` (${formatNumber(
                (server.ramUsed * 100.0) / server.maxRam,
                0,
                1,
              )}% used)`
            : "") +
  ...
  function getTreeDepth(server, stack = []) {
    ...
    const maxSubTreeDepth =
      1 + Math.max(...connectedServers.map((s) => getTreeDepth(s, stack)));
  ...
  doc
    ...
    .forEach((serverEntry) =>
      serverEntry.addEventListener(
        "click",
        setNavCommand.bind(null, routes[serverEntry.childNodes[0].nodeValue]),
      ),
    );
  doc
    ...
    .forEach((backdoorButton) =>
      backdoorButton.addEventListener(
        ...
        setNavCommand.bind(
          null,
          routes[
            backdoorButton.parentNode.childNodes[0].childNodes[0].nodeValue
          ] + ";backdoor",
        ),
      ),
    );
